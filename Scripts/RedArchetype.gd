extends Node
# ===================================================================
# RED ARCHETYPE — FINAL CLEAN VERSION (November 17, 2025)
# Fully commented, no errors, glow + cracks working
# ===================================================================

# ──────────────────────── PLAYER & VISUAL REFERENCES ────────────────────────
var player: CharacterBody2D                    # Main player node
var anim_sprite: AnimatedSprite2D              # Player animation sprite
var thread_line: Line2D                        # Grapple thread visual
var grapple_popup: Label                       # "Q to attach/release" prompt
var debug_label: Label                         # Debug text above player
var trail_container: Node2D                    # Holds stomp smear trails
...
# ──────────────────────── GLOW + CRACK CONTROL ────────────────────
@export var glow_lerp_speed: float = 8.0
@export var glow_activation_distance: float = 300.0
@export var glow_max_intensity: float = 3.0
var target_glow: float = 0.0
var current_glow: float = 0.0
var current_cracks: float = 0.0

# ──────────────────────── BREAKABLE PLATFORM SYSTEM ───────────────
var breakable_tilemap: TileMap
var breakable_shader: ShaderMaterial
var is_red_active: bool = false
const BREAKABLE_LAYER: int = 0
const TILE_WORLD_SIZE: Vector2 = Vector2(128, 128)  # 512px tile at 0.25 scale
...
# ===================================================================
# STOMP GLOW SHADER (player glow during stomp)
# ===================================================================
...
# ===================================================================
# BREAKABLE TILEMAP SHADER SETUP
# ===================================================================
func _setup_breakable_tilemap() -> void:
	if breakable_tilemap: return
	...
	if not tilemap_node or not (tilemap_node is TileMap):
		push_warning("RedBreakablePlat node NOT FOUND!")
		return
	...
	breakable_shader.set_shader_parameter("pulse_speed", 18.0)
	breakable_tilemap = tilemap_node
	print("[RED] Breakable TileMap ready")

# ===================================================================
# ACTIVATION (called when player switches to Red)
# ===================================================================
...
func _update_breakable_glow() -> void:
	...
	for coord in breakable_tilemap.get_used_cells(BREAKABLE_LAYER):
		...
		if not data or data.get_custom_data("red_breakable") != true:
			continue
		var tile_pos = breakable_tilemap.map_to_local(coord)
		var tile_dist = player_pos.distance_to(tile_pos)
		if tile_dist < glow_activation_distance:
			var t = 1.0 - (tile_dist / glow_activation_distance)
			var intensity = lerp(0.0, glow_max_intensity, t * t)
			max_intensity = max(max_intensity, intensity)

	target_glow = max_intensity
	current_glow = lerp(current_glow, target_glow, glow_lerp_speed * get_physics_process_delta_time())
	current_cracks = current_glow * 0.8

	breakable_shader.set_shader_parameter("glow_intensity", current_glow)
	breakable_shader.set_shader_parameter("crack_intensity", current_cracks)

# ===================================================================
# DOUBLE-TAP DOWN → STOMP
# ===================================================================
...
# ===================================================================
# BREAK WEAK TILES + CENTERED PARTICLES
# ===================================================================
func _break_weak_tiles() -> void:
	if not breakable_tilemap: return
	var foot_pos = player.global_position + Vector2(0, 16)
	...
	for dx in range(-1, 2):
		for dy in range(-1, 2):
			...
			if data and data.get_custom_data("red_breakable") == true:
				...
				break
		if platform_id_to_break != -1: break
	if platform_id_to_break == -1: return

	var broken_count = 0
	for coord in breakable_tilemap.get_used_cells(BREAKABLE_LAYER):
		var data = breakable_tilemap.get_cell_tile_data(BREAKABLE_LAYER, coord)
		if data and data.get_custom_data("platform_id") == platform_id_to_break:
			var tile_center = breakable_tilemap.map_to_local(coord) + TILE_WORLD_SIZE * 0.5
			breakable_tilemap.set_cell(BREAKABLE_LAYER, coord, -1)
			_spawn_break_particles(tile_center)
			broken_count += 1
...
# CHARGE DASH
# ===================================================================
func _handle_charge_dash(delta: float) -> void:
	if dash_cooldown_timer > 0.0:
		dash_cooldown_timer -= delta

	if connected and Input.is_action_just_pressed("Dash"):
	...
	if is_charging_dash:
		if Input.is_action_pressed("Dash"):
			...
			var duration = lerp(charge_dash_min_duration, charge_dash_max_duration, dash_charge)
			dash_timer = duration
			player.velocity = Vector2(last_direction * dash_speed, 0)
...
# ===================================================================
# GRAPPLE LOGIC — ONLY ONE FUNCTION (fixed duplicate + variable conflict)
# ===================================================================
...
func _find_unravel_target() -> Node2D:
	...
	return result.collider if result and result.collider.is_in_group("red_unravel") else null

func _unravel(t: Node2D) -> void:
	var tw = create_tween()
